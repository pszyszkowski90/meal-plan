...
| Artefakty nadrzędne         | NISKI | ŚREDNI | WYSOKI | Co zmienia się w stosunku do wartości bazowej                                                                                                              |
| --------------------------- | ----- | ------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
- Jeśli ramka **Pewność: NISKA** jest oznaczona, uwzględnij to w "Otwartych Ryzykach i Założeniach" planu i zadaj JEDNO pytanie wyjaśniające, jak postępować (najpierw zweryfikuj, lub zaplanuj z uznanym ryzykiem).
...
- Sekcja "Odwołania do kodu" JEST Twoim ugruntowaniem bazy kodu — nie odtwarzaj agentów Explore, aby znaleźć te same pliki.
...
6. **Zadawaj głębokie, dociekliwe pytania za pomocą AskUserQuestion**:
   ...
   **Przykład wywołania AskUserQuestion z rekomendacjami (poza oprogramowaniem — treść/strategia):** `Depth` to `[D]` — diagnostyka dotycząca odbiorców/zakresu; pomiń, jeśli brief ramowy już ustalił, dla kogo to jest.
   ...
   - question: "Jaki poziom szczegółowości technicznej powinien mieć moduł kursu?"
     ...
     - label: "Głębokie zanurzenie z otwartymi wyzwaniami"
       description: "Minimalne rusztowanie, problemy z prawdziwego świata. · Mocna strona: Wymusza prawdziwe rozwiązywanie problemów, najwyższe zapamiętywanie nauki. · Kompromis: Wysokie ryzyko rezygnacji dla mniej doświadczonych uczniów; trudniejsze do wsparcia na dużą skalę."
       multiSelect: false

   **O co pytać** — dostosuj kategorie do domeny zadania:

   Najpierw zidentyfikuj domenę zadania: **oprogramowanie**, **treści/edukacja**, **strategia/proces** lub **hybryda**. Następnie wybierz odpowiednie kategorie pytań. Poniższe kategorie są uporządkowane według domeny — wybierz to, co jest istotne, nie narzucaj kategorii oprogramowania zadaniom poza oprogramowaniem.
   ...
   **KRYTYCZNE**: MUSISZ zadać liczbę pytań odpowiednią do potwierdzonego poziomu złożoności *i* skalowania artefaktów nadrzędnych z Kroku 1.0. Nie skracaj tego, gdy nie dostarczono żadnych artefaktów nadrzędnych — dokładne pytania zapobiegają kosztownym przeróbkom. Podobnie, nie dodawaj pytań, gdy ramka lub badania już pokrywają temat — ponowne zadawanie pytań podważa zaufanie do artefaktu nadrzędnego. Każde pytanie powinno wymuszać prawdziwą decyzję, a nie potwierdzać coś oczywistego.
...
2. **Użyj tej struktury szablonu** (bloki faz zawierają zwykłe punktorowanie — `- ` zamiast `- [ ]` — a pojedyncza kanoniczna sekcja `## Progress` na dole jest właścicielem stanu pola wyboru, zobacz `references/progress-format.md` dla umowy):
...
> Krótki opis ram: `context/changes/<change-id>/frame.md` (jeśli istnieje — w przeciwnym razie pomiń wiersz)
...
4. `/10x-plan` nie zatwierdza własnych artefaktów; pozostaw zmianę w drzewie roboczym. Zostanie ona zatwierdzona później wraz z pierwszą fazą `/10x-implement` zmiany (która ponownie zmienia ten sam element na `in-progress`).
...
2. **Bądź interaktywny**:
   ...
   - Uzyskaj akceptację na każdym głównym etapie
...
6. **OBOWIĄZKOWE: Skalowane pytania pogłębione za pomocą AskUserQuestion**:
   ...
   - Każda opcja musi zawierać rekomendację `⭐ Recommended` z analizą mocnych stron/kompromisów
...
- **Uruchamiaj wiele zadań równolegle** w jednej wiadomości w celu równoczesnego wykonania
...
Planowanie może być obciążone kontekstem ze względu na badania + iteracje. Utrzymuj efektywność kontekstu:
...
- question: "Co użytkownik powinien widzieć podczas ładowania nowych elementów?"
  ...
  - label: "Spinner na całej stronie"
...
- question: "Co uczeń powinien być w stanie ZROBIĆ po tym module — nie tylko wiedzieć?"
  ...
  - label: "Ukończyć ćwiczenie z przewodnikiem"
    description: "Instrukcja krok po kroku z oczekiwanym wynikiem. · Mocna strona: Niska bariera — każdy kończy, buduje pewność siebie. · Kompromis: Może produkować 'tutorialowe zombie', które potrafią podążać, ale nie potrafią samodzielnie zastosować."
  - label: "Zdać test wiedzy"
    ...
    multiSelect: false
- question: "Jak ten moduł powinien radzić sobie z różnymi poziomami umiejętności w grupie docelowej?"
  header: "Poziomy"
  options:
  ...
  - label: "⭐ Recommended: Warstwowa głębokość"
    description: "Główna ścieżka, którą wszyscy podążają + opcjonalne sekcje pogłębione. · Mocna strona: Każdy otrzymuje wartość; zaawansowani uczniowie sami wybierają trudniejszy materiał. · Kompromis: Więcej treści do utrzymania; ryzyko ignorowania 'opcjonalnych' sekcji."
  - label: "Oddzielne ścieżki dla początkujących/zaawansowanych"
    description: "Dwie równoległe ścieżki rozchodzące się wcześnie. · Mocna strona: Każda grupa docelowa otrzymuje idealnie dopasowaną treść. · Kompromis: 2x koszt produkcji; podział małej kohorty może zaszkodzić dynamice społeczności."
    multiSelect: false

### Przykład 3: Strategia / Proces — złożoność ŚREDNIA (np. przepływ pracy biuletynu)

`Bottleneck` to `[D]` — czyste ramy problemu (jaki problem rozwiązać). To jest dokładnie rodzaj pytania, które ramka ma na celu rozstrzygnąć. Z briefem ramowym, pomiń to całkowicie; wiodąca hipoteza jest wąskim gardłem.
...
- question: "Jakie jest główne wąskie gardło w obecnym procesie biuletynu?"
  ...
  - label: "⭐ Recommended: Kuracja trwa zbyt długo"
    description: "Znajdowanie i ocenianie linków jest wolnym krokiem. · Mocna strona: Bezpośrednio celuje w czas do publikacji — automatyzacja kuracji daje największe oszczędności czasu na podstawie obecnych czasów procesu. · Kompromis: Zautomatyzowana kuracja ryzykuje utratę osobistego głosu redakcyjnego, który cenią subskrybenci."
  - label: "Pisanie komentarzy"
    description: "Linki są gotowe, ale pisanie wokół nich jest wolne. · Mocna strona: Projektowanie wspomagane AI może skrócić to o połowę. · Kompromis: Intensywne projektowanie AI może sprawić, że biuletyn będzie wydawał się generyczny — wymaga starannej kalibracji głosu."
  - label: "Dystrybucja i planowanie"
    description: "Treść jest gotowa, ale publikacja jest ręczna. · Mocna strona: Najłatwiejsze do zautomatyzowania — jasne dane wejściowe i wyjściowe. · Kompromis: Najmniejszy wpływ, jeśli kuracja lub pisanie nadal jest wąskim gardłem."
    multiSelect: false
