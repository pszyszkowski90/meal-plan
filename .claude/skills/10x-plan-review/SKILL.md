...
1. Argument wskazuje na zapisany plik przeglądu (zawiera `<!-- PLAN-REVIEW-REPORT -->`) → **wznowienie sortowania** (przejdź do kroku 6)
...
- **Luka w obietnicy**: każda zdolność obiecana w Pożądanym Stanie Końcowym / Kryteriach Sukcesu / Notatkach Migracyjnych powinna mieć fazę wspierającą. Jeśli kryteria sukcesu mówią „ograniczanie szybkości działa”, ale żadna faza tego nie buduje, implementator napotka lukę w trakcie budowy.
...
### Martwe punkty
Czego plan nie uwzględnił? Ścieżki błędów (opisana tylko ścieżka sukcesu?), historia wycofywania (faza 3 zawodzi — czy możemy cofnąć?), wpływ zasobów/kosztów (wywołania API, praca obliczeniowa — ile to kosztuje przy oczekiwanym użyciu?), zmiany wartości domyślnych (wartość domyślna, która potraja koszt lub czas, powinna być wyróżniona), luki w testowaniu, granice bezpieczeństwa.
...
Posortuj ustalenia według wagi: KRYTYCZNA → OSTRZEŻENIE → OBSERWACJA. Ogranicz do 10 — skonsoliduj powiązane ustalenia, jeśli masz ich więcej.
...
═══════════════════════════════════════════════════════════
  ...
  ╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
    ...
    Poprawka A ⭐ Zalecana: Uczyń kolumnę dopuszczalną + oddzielne, restartowalne uzupełnianie
      ...
      Kompromis:   Dwa wdrożenia (dodaj dopuszczalne → uzupełnij → wymuś NOT NULL).
      Pewność: WYSOKA — to dokładnie to podejście zostało czysto wdrożone 3 miesiące temu.
      Martwy punkt: Krok wymuszenia nadal potrzebuje własnej notatki o wycofaniu.
...
- Waga, Wpływ, Wymiar, Lokalizacja są na osobnych liniach z wyrównanymi etykietami. Szczegóły zaczynają się na osobnej linii pod etykietą `Detail:`, dzięki czemu mogą naturalnie zawijać.
...
Przejdź przez ustalenia w kolejności ważności (KRYTYCZNA → OSTRZEŻENIE → OBSERWACJA). Dla każdego:
